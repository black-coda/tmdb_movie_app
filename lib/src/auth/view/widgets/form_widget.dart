...
class DynamicInputWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      style: const TextStyle(color: Konstant.white),
      ...
      decoration: InputDecoration(
        enabledBorder: const OutlineInputBorder(
          borderSide: BorderSide(color: Konstant.formColor),
        ),
        focusedBorder: const OutlineInputBorder(
            borderSide: BorderSide(color: Color(0xff4964AC))),
        border: const OutlineInputBorder(
            // Make border edge circular
            borderRadius: BorderRadius.all(Radius.circular(10.0)),
            borderSide: BorderSide(color: Color(0xff4964AC))),
        fillColor: const Color(0xff050A31),
        filled: true,
        label: Text(labelText),
        labelStyle: const TextStyle(color: Konstant.formColor),
        prefixIcon: prefIcon,
...
class AuthValidators {
  ...
  static const String defaultMsg = "Field cannot be empty";

  String? defaultValidator(String? val) {
    String value = val as String;
    if (value.isEmpty) return defaultMsg;
